...
const server = new index_js_1.Server({ name: 'meowdel', version: '2.0.0' }, { capabilities: { tools: {} } });
server.setRequestHandler(types_js_1.ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: 'search_brain',
            description: 'Semantic search through Meowdel Brain — a personal knowledge base of notes, ' +
        ...
        {
            name: 'get_note',
            description: 'Read the full content of a specific Brain note by its slug. ' +
        ...
        {
            name: 'list_notes',
            description: 'List all notes in Meowdel Brain with titles, tags, and word counts. ' +
        ...
        {
            name: 'ask_meowdel',
            description: 'Ask a Meowdel AI cat personality a question. These cats are opinionated AI advisors ' +
...
server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
    ...
    try {
        if (name === 'search_brain') {
            ...
            const results = await (0, api_1.searchNotes)(query, limit);
            if (!results.length) {
                return { content: [{ type: 'text', text: 'No notes found matching that query.' }] };
            }
            const text = results.map(n => `**${n.title}** (slug: \`${n.slug}\`)\n` +
                (n.summary ? `${n.summary}\n` : '') +
                `Tags: ${(n.tags || []).join(', ') || 'none'} · ${n.wordCount}w`).join('\n\n');
        ...
        if (name === 'list_notes') {
            ...
            const text = notes.map(n => `- **${n.title}** (\`${n.slug}\`) · ${n.wordCount}w` +
                (n.tags?.length ? ` · #${n.tags.join(' #')}` : '')).join('\n');
            return { content: [{ type: 'text', text: `${notes.length} note(s):\n\n${text}` }] };
        }
        if (name === 'ask_meowdel') {
            const question = String(args?.question ?? '');
            const pet = String(args?.pet ?? 'bandit');
            const res = await (0, api_1.chatRequest)(question, pet);
        ...
        const msg = (0, api_1.apiError)(err);
...
async function main() {
    ...
    if (!hasKey) {
        process.stderr.write('Meowdel MCP: No API key found. Run `meowdel config` or set MEOWDEL_API_KEY.\n');
        process.exit(1);
    }
